...
import { TransactionSidebar } from '../../../shared/components/transaction-sidebar/transaction-sidebar';
...
type FilterKeys = keyof typeof Deposit.prototype.filterCriteria;
...
export class Deposit {
  public headerInformation = {
    ...
    pageSubtitle: 'Texto de apoio',
    breadcrumb: [
      { label: 'Painel', path: '/dashboard' },
      { label: 'Transacões', path: '/transacoes' },
      { label: 'Pagamento', path: '/transacoes/deposit' }
    ],
  }
  ...
  private allDeposits: IDeposit[] = [];
  public filteredDeposits: IDeposit[] = [];
  public paginatedDeposits: IDeposit[] = [];
  ...
  private generateMockData(): void {
    this.allDeposits = Array.from({ length: 25 }, (_, i) => ({
      beneficiario: {
        id: `8310000${37 + i}`,
  ...
  applyFilters(): void {
    let tempDeposits = [...this.allDeposits];

    if (this.searchTerm) {
      const lowerCaseSearch = this.searchTerm.toLowerCase();
      tempDeposits = tempDeposits.filter(p => 
    ...
    if (this.filterCriteria.cliente) {
        ...
        tempDeposits = tempDeposits.filter(p => p.beneficiario.id.toLowerCase().includes(lowerCaseCliente));
    }
     if (this.filterCriteria.criterio) {
        const lowerCaseCriterio = this.filterCriteria.criterio.toLowerCase();
        tempDeposits = tempDeposits.filter(p => p.pagador.wallet.toLowerCase().includes(lowerCaseCriterio));
    }

    this.filteredDeposits = tempDeposits;
